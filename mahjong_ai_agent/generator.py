...
# プロンプトテンプレート
MAHJONG_QUESTION_PROMPT = """麻雀の点数計算問題の問題文を生成してください。できるだけユニークで多様な状況を作成してください。
...
class QuestionGenerator:
    ...
    async def generate_question(
        ...
        Args:
            difficulty: 難易度 (easy, medium, hard)。instructionが指定されていない場合に使用
        ...
        try:
            ...
            for i in range(num_questions):
                if not instruction:
                    inst = f"難易度{difficulty}で、バリエーション{i+1}として前の問題とは異なる牌の組み合わせと役を使用した問題を作成してください。"
                else:
                    inst = instruction
